...
class TutorTestBase(unittest.TestCase):
    ...
    def mastery(self, kind, key):
        rows = appdb.query(
            ...
            if False
            else f"SELECT * FROM mastery WHERE item_kind='{kind}' AND item_key='{key}'"
        )
...
class RecordTest(TutorTestBase):
    ...
    def test_attempt_created_at_full_ts_and_mastery_stamps(self):
        self.seed_grammar()
        with (
            mock.patch("tutor._today", return_value="2026-06-12"),
    ...
    def test_leitner_transitions(self):
        ...
        def rec(verdict):
            with mock.patch("tutor._today", return_value="2026-06-12"):
                run_main(
                    ["record", "grammar", "article-omission", "fix", verdict],
                    stdin_text="x",
                )
    ...
    def test_vocab_fail_triggers_relearn(self):
        ...
        run_main(
            ["record", "vocab", "throughput", "reverse", "fail"],
    ...
    def test_bad_verdict_or_kind_is_error(self):
        self.assertEqual(
            run_main(["record", "grammar", "k", "fix", "maybe"], stdin_text="x")[0], 1
        )
        self.assertEqual(
            run_main(["record", "nosuch", "k", "fix", "pass"], stdin_text="x")[0], 1
        )
