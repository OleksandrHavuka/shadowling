...
4. Analyze each pending entry. For every finding **whose normalized key is not
   ...
   - `rephrasings` — "yours", "natural", why  *(natural-sounding native phrasing / collocations; add a `native_language` gloss in `why` if helpful)*
   - `idioms` — context, idiom, meaning (in `native_language`), "you wrote"
