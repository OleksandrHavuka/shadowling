---
name: vipe
description: "Dev: wipe the debrief product/log docs from SHADOWLING_HOME for a clean test run; keeps config.json, words.csv, the buffer and the raw message log. Usage: /vipe"
...
slate. Keeps `config.json`, `words.csv`, the current `buffer.jsonl`, and the raw
corpus `messages.log.jsonl`. Deletion is an explicit list of file names (brace
expansion), never a wildcard or `rm -rf`.
