...
class SessionVerbsTest(CaptureTestBase):
    ...
    def test_capture_stores_session_id(self):
        rows = capture.query("SELECT session_id FROM messages ORDER BY id")
        self.assertEqual(
            [r["session_id"] for r in rows], ["sess-A", "sess-B", "sess-A"]
        )
    ...
    def test_sessions_skips_drill_only_sessions(self):
        with closing_con() as con:
            with con:
                con.execute(
                    "UPDATE messages SET kind='drill' WHERE session_id='sess-B'"
                )
        self.assertEqual([s["session"] for s in capture.sessions()], ["sess-A"])
    ...
    def test_mark_processed_session_scoped_and_stamps_drills(self):
        ...
        with closing_con() as con:
            with con:
                con.execute("UPDATE messages SET kind='drill', langs=NULL WHERE id=3")
...
class MarkDrillsTest(CaptureTestBase):
    ...
    def test_short_lookalike_below_threshold(self):
        ...
        capture.mark_drills()
        self.assertEqual(capture.query("SELECT kind FROM messages")[0]["kind"], None)

    def test_similarity_characterization(self):
        similarity = capture._similarity
        self.assertGreaterEqual(similarity("I have gone home", "I have gone home"), 1.0)
        self.assertGreaterEqual(
            similarity("I have gone home", "I Have Gone Home!"), 0.9
        )
        self.assertGreaterEqual(similarity("take  a   photo", "take a photo"), 0.9)
        self.assertLess(similarity("went, gone", "went home"), 0.9)
        self.assertLess(similarity("I have gone home", "the deploy is green now"), 0.5)
